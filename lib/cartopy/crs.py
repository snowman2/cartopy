...
def _safe_pj_transform_611(src_crs, tgt_crs, x, y, z=None):
    """
    ...
    transformed_coords = transformer.transform(x, y, z)
    if z is None:
...
def _safe_pj_transform_pre_611(src_crs, tgt_crs, x, y, z=None):
    transformer = Transformer.from_crs(src_crs, tgt_crs, always_xy=True)
    transformed_coords = transformer.transform(x, y, z)
    if z is None:
        xx, yy = transformed_coords
    ...
    def transform_points(self, src_crs, x, y, z=None):
        ...
        if npts:
            result[:, 0], result[:, 1], result[:, 2] = \
                _safe_pj_transform(src_crs, self, x, y, z)
...
class Projection(CRS, metaclass=ABCMeta):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not self.is_projected:
            raise ValueError('Input does not define a projection.')
        self.bounds = None
        if self.area_of_use:
