...
cdef class SphericalInterpolator(Interpolator):
    ...
    cdef void init(self, src_crs, dest_crs) except *:
        ...
        cdef double inverse_flattenting = src_crs.ellipsoid.inverse_flattening

        geod_init(&self.geod, major_axis, inverse_flattenting)
    ...
    cdef Point project(self, const Point &lonlat) except *:
        ...
        dest_x = lonlat.x * self.src_scale
        dest_y = lonlat.y * self.src_scale

        xx, yy = self.transformer.transform(
            dest_x * self.src_scale,
            dest_y * self.src_scale,
        )
